...
- **Admins can promote a Student to Counselor directly** — no need to create a new account; the Admin dashboard's student list has a one-click "Promote to Counselor" button on each student.
- **Admins can demote a Counselor back to Student** — the reverse of the above, in a new "Staff Accounts" list in the Admin dashboard.
- **Blog posts (Teachings) can now be deleted** — a "Manage Teachings" list in the Admin/Counselor dashboard lets Counselors delete their own posts, and Admins delete any post, by anyone.
- **Multi-module course builder** — creating a course now lets you add as many modules as you want, each with as many lessons as you want, each lesson with its own title, description, and video upload (with a real progress percentage) — all before publishing, in one form.
- **Groups support photos, videos, and voice notes** — not just text. Posting inside a group now has the same upload buttons as the main Zion Digital City feed.
